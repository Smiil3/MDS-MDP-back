...
const bookingSelect = {
  ...
  created_at: true,
  updated_at: true,
  booking_status: { select: { id_booking_status: true, label: true } },
  mechanic: { select: { id_mechanic: true, name: true, city: true, address: true } },
  driver: {
    select: {
      id_driver: true,
      last_name: true,
...
export class BookingRepository {
  findById(id: number) {
    return prisma.booking.findUnique({ where: { id_booking: id }, select: bookingSelect });
  }
  ...
  findServicePrices(serviceIds: number[]) {
    return prisma.garage_service.findMany({
      where: { id_garage_service: { in: serviceIds } },
    ...
    return prisma.$transaction(async (tx) => {
      return tx.booking.create({
        data: {
          ...
          booking_garage_service: {
            create: data.service_ids.map((id) => ({ id_garage_service: id })),
          },
        },
        select: bookingSelect,
      });
  ...
  delete(id: number) {
    return prisma.booking.delete({ where: { id_booking: id } });
  }
}
